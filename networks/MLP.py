...
class MLP(nn.Module):
    def __init__(self, hidden_size=config.MLP.SIZE, num_targets=config.MLP.classes):
        super(MLP, self).__init__()
        transformers = config.use_transformers
        input_size = config.BERT_DIM * 2 if transformers else config.W2V_SIZE*2
