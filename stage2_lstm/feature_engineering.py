...
class DataSet():
    ...
    def extract_features(self, df, phase):
        ...
        if os.path.exists(feature_path) and os.path.exists(stance_path):
            ...
            text_feats = self.convert_to_vector(headlines, articles, phase)
            text_feats = text_feats['matrix']
    ...
    def convert_to_vector(self, headlines, articles, name):
        if name == 'train':
